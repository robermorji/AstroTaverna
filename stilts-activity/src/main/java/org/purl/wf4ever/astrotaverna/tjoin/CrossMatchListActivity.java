...
//------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------
// THIS CODE IS NOT FINISHED --> YOU SHOULD CONTINUE 
// CREATING THE PARAMETERS FOR THE MAIN METHOD 
//------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------

public class CrossMatchListActivity extends
		AbstractAsynchronousActivity<CrossMatchListActivityConfigurationBean>
		implements AsynchronousActivity<CrossMatchListActivityConfigurationBean> {
	...
	private static final String IN_VALUES = "valuesList";
	private static final String IN_TUNING = "tuning";
	private static final String IN_IREF = "iref";
	...
	private CrossMatchListActivityConfigurationBean configBean;

	
	private static Logger logger = Logger.getLogger(CrossMatchListActivity.class);
	
	@Override
	public void configure(CrossMatchListActivityConfigurationBean configBean)
		...
		if(!(configBean.getJoin().compareTo("default")==0
				|| configBean.getJoin().compareTo("match")==0
				|| configBean.getJoin().compareTo("nomatch")==0
				|| configBean.getJoin().compareTo("always")==0)){
			throw new ActivityConfigurationException(
		...
		if(!(configBean.getMultimode().compareTo("pairs")==0
				|| configBean.getMultimode().compareTo("group")==0)){
			throw new ActivityConfigurationException(
					"Invalid multimode parameter.");
		}
	...
	protected void configurePorts() {
		...
		addInput(IN_FIRST_INPUT, 1, true, null, String.class);
		...
		addInput(IN_VALUES, 1, true, null, String.class);
		
		if(configBean.getMatchCriteria().compareTo("exact")!=0){
			addInput(IN_TUNING, 0, true, null, String.class);
			addInput(IN_PARAMS, 0, true, null, String.class);
		}
		
		if(configBean.getMultimode().compareTo("pairs")==0){
			addInput(IN_IREF, 0, true, null, String.class);   //OPTIONAL PARAMETER
		}
		...
		callback.requestRun(new Runnable() {
			...
			public void run() {
				...
				if(areMandatoryInputsNotNull()){
					...
					// RESOLVE INPUTS			
					firstInput = (List<String>) referenceService.renderIdentifier(inputs.get(IN_FIRST_INPUT), 
							String.class, context);
					valuesList = (List<String>) referenceService.renderIdentifier(inputs.get(IN_VALUES), 
							String.class, context);
					...
					if(configBean.getMultimode().compareTo("pairs")==0){
						pairmode = true;
						if(inputs.get(IN_IREF)==null)
							iref = 1;
						else
							try{
								iref = (new Integer ((String) referenceService.renderIdentifier(inputs.get(IN_IREF), 
									String.class, context))).intValue();
							}catch (Exception ex){
								callbackfails = true;
								logger.error(inputs.get(IN_IREF) + " couldn't be parsed to int");
								callback.fail(inputs.get(IN_IREF) + " couldn't be parsed to int");
					...
					if(configBean.getTypeOfInput().compareTo("File")==0){
						if(firstInput.size()<2){
							callback.fail("CatList service requires a list with at least two votable. ");
							callbackfails = true;
						}
						for(int i=0; i< firstInput.size();i++){
							File file = new File(firstInput.get(i));
							tmpFiles.add(file);
					...
					if(configBean.getTypeOfInput().compareTo("String")==0){
						try{
							for(int i = 0; i<firstInput.size(); i++){
								File file = MyUtils.writeStringAsTmpFile(firstInput.get(i));
								file.deleteOnExit();
								firstInput.set(i, file.getAbsolutePath());
							}
						}catch(Exception ex){
					...
					numTables = firstInput.size();
					if(valuesList.size() != firstInput.size()){
						logger.error("Size of votableList and valuesList doesn't match. ");
						callbackfails = true;
						callback.fail("Size of votableList and valuesList doesn't match. ");
					}
					...
					if(!callbackfails){
                        ...
                        */
						...
						for(int i = 1; i< valuesList.size(); i++)
							parametersList.add("values"+i+"="+valuesList.get(i));
	...
	@Override
	public CrossMatchListActivityConfigurationBean getConfiguration() {
