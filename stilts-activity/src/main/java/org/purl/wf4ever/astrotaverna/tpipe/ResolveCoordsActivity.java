 ...
 * @since    19 May 2011
 */
public class ResolveCoordsActivity extends
		AbstractAsynchronousActivity<ResolveCoordsActivityConfigurationBean>
		implements AsynchronousActivity<ResolveCoordsActivityConfigurationBean> {
	...
	@Override
	public void configure(ResolveCoordsActivityConfigurationBean configBean)
	...
	protected void configurePorts() {
		...
		addInput(IN_RA_COLUMN, 0, true, null, String.class);
		addInput(IN_DEC_COLUMN, 0, true, null, String.class);
		addInput(IN_COL_ID_OBJ_NAME, 0, true, null, String.class);
		...
		callback.requestRun(new Runnable() {
			...
			public boolean areMandatoryInputsNotNull(){
				...
				if(inputs.get(IN_FIRST_INPUT_TABLE)==null
						|| inputs.get(IN_RA_COLUMN)==null
						|| inputs.get(IN_DEC_COLUMN)==null
						|| inputs.get(IN_COL_ID_OBJ_NAME)==null)
					validStatus = false;
			...
			public void run() {
				...
				if(areMandatoryInputsNotNull()){
					...
					String ra_column = (String) referenceService.renderIdentifier(inputs.get(IN_RA_COLUMN), String.class, context);
					String dec_column= (String) referenceService.renderIdentifier(inputs.get(IN_DEC_COLUMN), String.class, context);
					String id_column = (String) referenceService.renderIdentifier(inputs.get(IN_COL_ID_OBJ_NAME), String.class, context);
	...
	//				}
					...
					if(!callbackfails){
						...
						parameters[4] = "cmd=addresolve " + id_column + " "+ ra_column + " " + dec_column;
	...
	@Override
	public ResolveCoordsActivityConfigurationBean getConfiguration() {
