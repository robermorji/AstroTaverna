...
public class CoordTransformationActivityHealthChecker implements
	...
	public boolean canVisit(Object o) {
		...
		return o instanceof CoordTransformationActivity;
	}
	...
	public VisitReport visit(CoordTransformationActivity activity, List<Object> ancestry) {
		CoordTransformationActivityConfigurationBean config = activity.getConfiguration();
		...
		*/
		...
		if(! activity.isAllowedCoordenatesFunction(config.getTypeOfFilter())){
			subReports.add(new VisitReport(HealthCheck.getInstance(), activity,
					"Invalid coordenates function.", HealthCheck.INVALID_CONFIGURATION,
					Status.WARNING));
		}
